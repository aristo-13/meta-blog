 ...
 useEffect(()=> {
  ...
  return (
    <main>
      ...
      <div className='w-[90%] h-[500px] bg-slate-400/20 mx-auto rounded-xl relative mb-10 bg-cover' style={{backgroundImage: `url(${import.meta.env.VITE_IMG_URL+data[0]?.attributes?.img?.data?.attributes?.url})`}}>
          <div className="w-[90%] h-[200px] md:w-[400px] md:h-[250px] bg-white dark:bg-[#181A2A] absolute bottom-2 left-2 md:-bottom-5 md:left-10 border-2 rounded-2xl shadow-xl flex  flex-col p-2" >
                  <div className='w-full  text-blue-950 p-2'>
                     <span className='bg-blue-700 text-white px-3 py-[2px] top-[2px] left-1 rounded'>{data[0]?.attributes?.Category}</span>
                      <p className='text-blue-950 p-2 dark:text-white font-bold'>{(data[0]?.attributes?.summery)?.substring(0,150) + '...'}</p>
                  </div>
            <div className='w-full absolute bottom-2 left-0 flex text-blue-950 justify-between items-center p-2'>
                <div className='flex gap-2 items-center'>
                      <div className='w-[25px] h-[25px] rounded-full bg-cover border bg-gray-400' style={{backgroundImage: `url(${import.meta.env.VITE_IMG_URL+data[0]?.attributes?.author?.data?.attributes?.avatar?.data?.attributes?.url})`}}></div>
                      <h1 className='text-[0.8rem] text-gray-500'>{data[0]?.attributes?.author?.data?.attributes?.name || 'User'}</h1>
                </div>
